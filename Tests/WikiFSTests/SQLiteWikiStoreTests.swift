...
struct SQLiteWikiStoreTests {
    ...
    @Test func pragmasAndSchema() throws {
        ...
        #expect(tables.isSuperset(of: ["pages", "attachments", "page_links", "ingested_files"]))
        ...
        // user_version guard: a fresh DB runs both migration steps → version 2;
    ...
    @Test func changeTokenAdvancesOnEveryMutation() throws {
        ...
        // Phase 5 format: "<pCount>:<pSum>:<fCount>:<fSum>". Empty DB → "0:0:0:0".
        #expect(try store.changeToken() == "0:0:0:0")

        // Create bumps page COUNT and SUM (version starts at 1) → "1:1:0:0".
        let a = try store.createPage(title: "Alpha")
        #expect(try store.changeToken() == "1:1:0:0")

        // Update bumps that row's version by 1 → SUM increments → "1:2:0:0".
        try store.updatePage(id: a.id, title: "Alpha", body: "edited")
        #expect(try store.changeToken() == "1:2:0:0")
        ...
        #expect(try store.changeToken() == "2:3:0:0")

        try store.updatePage(id: b.id, title: "Beta", body: "beta edit")
        #expect(try store.changeToken() == "2:4:0:0")

        // Delete changes page COUNT and SUM → "1:2:0:0".
    ...
    @Test func changeTokenAdvancesOnIngestAndDelete() throws {
        ...
        // Ingest bumps the file COUNT and SUM (version 1) → "0:0:1:1".
        let f = try store.ingestFile(filename: "a.txt", data: Data("hi".utf8))
        #expect(try store.changeToken() == "0:0:1:1")

        // A second file → "0:0:2:2".
        _ = try store.ingestFile(filename: "b.txt", data: Data("yo".utf8))
        #expect(try store.changeToken() == "0:0:2:2")

        // Delete the first → file COUNT and SUM drop → "0:0:1:1".
