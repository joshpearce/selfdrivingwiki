...
| [`plans/link-context-menus.md`](plans/link-context-menus.md) | **Design (future PR).** Right-click link context menus (Suggest for missing links, Find Similar for any link, Copy as wiki-link / file path, Open in Browser + Edit Link). Documents the Textual blocker (its `NSTextInteractionView` owns right-click + the menu internally; `model.url(for:)` hit-test is internal) and the approved decision to vendor Textual in-repo for that PR. |
