...
| [`plans/zotero-integration.md`](plans/zotero-integration.md) | **In progress:** browsing a Zotero library from inside the app and ingesting a PDF/Markdown attachment through the existing ingest pipeline. PR1 (core: `ZoteroClient`, local-storage resolution, config, Keychain credential store, `WikiStoreModel.ingestFromZotero`) is landed; PR2 (Settings UI + picker sheet) is not yet built. |
