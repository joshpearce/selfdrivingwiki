...
| [`plans/textual-to-wkwebview.md`](plans/textual-to-wkwebview.md) | **Design (not yet implemented).** Replace the vendored Textual reader with the WKWebView + `MarkdownHTMLRenderer` path across **every** reader surface (pages, sources, system prompt, changelog, agent transcript), then delete `Packages/Textual/` entirely. Generalizes `SourceWebView` → `WikiReaderView`, fills the two real gaps (ghost-link coloring, ⌘+ zoom) and fixes the `SourceWebView.route` `wiki://` bug. As part of the migration the agent transcript gains `[[wiki-link]]` support (linkify pre-pass + `onWikiLink` routing closure). |
